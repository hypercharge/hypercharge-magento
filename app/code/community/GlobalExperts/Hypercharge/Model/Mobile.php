...
class GlobalExperts_Hypercharge_Model_Mobile extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function initialize($paymentAction, $stateObject)
    {
        ...
        if ($initialize->unique_id && $initialize->status == 'new' && !$initialize->error) {
            ...
            $infoInstance->setAdditionalInformation('unique_id', $uniqueTransactionToken);
            $infoInstance->setAdditionalInformation('transaction_id', $transactionToken);
            $infoInstance->setAdditionalInformation('redirect_url', $redirectUrl);
            $infoInstance->setAdditionalInformation('cancel_url', $cancelUrl);
            ...
            $paymentInstance = $this->getInfoInstance()->getOrder()->getPayment();
            $paymentInstance->setAdditionalInformation('unique_id', $uniqueTransactionToken);
            $paymentInstance->setAdditionalInformation('transaction_id', $transactionToken);
            $paymentInstance->setAdditionalInformation('redirect_url', $redirectUrl);
            $paymentInstance->setAdditionalInformation('cancel_url', $cancelUrl);
    ...
    public function hyperResponse($post)
    {
        ...
        try {
            ...
            Mage::helper('bithypercharge')->logger("\n" . $timestamp . ' Transaction could not be found in database - ' . $pay_trx_id . '. Error: ' . $e->getMessage());
            return $xml;
        }
        $paymentMethod = get_class($order->getPayment()->getMethodInstance());
        if ($paymentMethod) {
        ...
        if (!$pass && function_exists('hash')) { // the signature doesn't match
            if (!$mode) {
                Mage::helper('bithypercharge')->logger("\n" . $timestamp . ' Invalid signature. Signature: ' . $signature . ' Payment id: ' . $pay_id . ' Password: ' . $channel['pass']);
            }
            return $xml;
        }
        ...
        $transactionType = ($transaction && $transaction->transaction_type) ? $transaction->transaction_type : $trx_type;
        $transactionId = ($transaction && $transaction->unique_id)  ? $transaction->unique_id : $pay_id;
        $transactionAmount = ($transaction && $transaction->amount) ? $transaction->amount : $response->amount;
        $wire_reference_id = ($transaction && $transaction->wire_reference_id) ? $transaction->wire_reference_id : "";
